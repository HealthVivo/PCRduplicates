...
int compare_read_SNP(struct alignedread* read,VARIANT* varlist,int ss,int start,int l1,int l2,FRAGMENT* fragment)
{
	...
	if (match != '-' && varlist[ss].type ==0) 
	{
		fragment->alist[fragment->variants].varid = ss; fragment->alist[fragment->variants].qv = read->quality[l1+offset];
		if (read->quality[l1+offset]-QVoffset >= MINQ) fragment->alist[fragment->variants].allele = match; 
		else fragment->alist[fragment->variants].allele = '?';
		fragment->variants++;

		if (!(read->flag & 1024) && read->quality[l1+offset]-QVoffset >= MINQ) // do not update allele counts for duplicate marked reads 02/22/2016 
		{
			varlist[ss].depth++; 
			if (match == '0') 
			{
		...
		if (match == '-' && varlist[ss].type ==0 && strlen(varlist[ss].allele1) == 1) 
		{
			fragment->alist[fragment->variants].varid = ss; fragment->alist[fragment->variants].allele = '?';
			fragment->alist[fragment->variants].qv = read->quality[l1+offset];
			fragment->variants++;
...
{
	int start = read->position; int end = start + read->span; 

	#ifdef SOFT_CLIP_EXTEND
	{
		if ((read->cigarlist[0]&0xf) == BAM_CSOFT_CLIP) start -= read->cigarlist[0]>>4; // soft clip at first position 
		if ((read->cigarlist[read->cigs-1]&0xf) == BAM_CSOFT_CLIP) end += read->cigarlist[read->cigs-1]>>4; // soft clip at last position 
	}
	#endif

	int ss=0,firstvar=0,lastvar=0,j=0,ov=0,i=0;
	j = (int)(start/BSIZE); 
	...
	for (i=0;i<read->cigs;i++)
	{
		...
		else if (op == BAM_CREF_SKIP) l2 += ol;  // N cigar, like deletion
		else if (op == BAM_CSOFT_CLIP) 
		{
			// we need to output variants that overlap the soft-clipped part with missing allele since it does overlap but information is not present 
			// almost like 'M' cigar but no matching is done... 
			#ifdef SOFT_CLIP_EXTEND
				while (varlist[ss].position >= start+l2 && varlist[ss].position < start+l2 + ol && ss <= chromvars[chrom].last)
				{
					...
					{
						fragment->alist[fragment->variants].varid = ss; 
						fragment->alist[fragment->variants].qv = 'A';
						fragment->alist[fragment->variants].allele = '?'; 
						fragment->variants++;
						//fprintf(stderr,"adding dummy variant %d \n",ss); 
					}
					ss++;
				}	
				l2 += ol; 
